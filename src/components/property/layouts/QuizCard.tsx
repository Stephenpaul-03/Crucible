...
export function QuizCard({ quizzes }: QuizCardProps) {
  ...
  return <div className="mt-12 max-w-3xl rounded-xl border border-zinc-200 bg-zinc-50/50 p-6 shadow-xl backdrop-blur-md dark:border-white/[0.08] dark:bg-white/[0.03]">
    <div className="flex items-center justify-between gap-4 border-b border-zinc-200/60 pb-4 dark:border-white/[0.05]"><div className="flex items-center gap-2.5"><div className="flex size-7 items-center justify-center rounded-lg bg-zinc-100 text-zinc-800 dark:bg-white/[0.06] dark:text-zinc-50"><HelpCircle className="size-4" /></div><div><h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Project Check</h3><p className="text-[10px] text-zinc-500 dark:text-zinc-400">{quizzes.length > 1 ? `${questionIndex + 1} of ${quizzes.length} checks` : "Review this checkpoint before moving on."}</p></div></div>{quizzes.length > 1 && <span className="font-mono text-[10px] text-zinc-500">{String(questionIndex + 1).padStart(2, "0")}/{String(quizzes.length).padStart(2, "0")}</span>}</div>
    {complete ? <div className="py-8 text-center"><CheckCircle2 className="mx-auto size-8 text-zinc-700 dark:text-zinc-200" /><h4 className="mt-3 text-base font-semibold text-zinc-900 dark:text-zinc-100">Project check complete</h4><p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Every checkpoint has been reviewed.</p><button type="button" onClick={handleReset} className="mt-5 rounded-md border border-zinc-200 px-4 py-2 text-xs font-semibold text-zinc-700 transition-colors hover:bg-zinc-100 dark:border-white/[0.08] dark:text-zinc-300 dark:hover:bg-white/[0.05]">Review again</button></div> : <>
