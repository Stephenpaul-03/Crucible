...
export function GlobalHomePage({ onSelectWorkspace, showWelcome, onDismissWelcome, onOpenContact }: GlobalHomePageProps) {
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return SUBJECTS.filter((subject) =>
      `${subject.label} ${subject.id} ${subject.tags.join(" ")}`
        .toLowerCase()
    ...
    <div className="touch-scroll-y h-screen overflow-y-auto bg-zinc-50 text-zinc-950 antialiased dark:bg-[#050608] dark:text-zinc-100">
      <header className="flex h-16 items-center justify-between border-b border-zinc-200 bg-white/80 px-5 backdrop-blur-2xl dark:border-white/[0.08] dark:bg-[#07080b]/78 md:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-zinc-950 dark:text-zinc-50">Crucible</p>
            <p className="text-[11px] text-zinc-500">
              <button type="button" onClick={onOpenContact} className="font-semibold text-zinc-700 underline decoration-dotted underline-offset-2 transition-colors hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-zinc-50" aria-label="Contact Stephen">
                Stephen
              </button>
              ’s hardware tinkering journal
            </p>
          </div>
        </div>
        <ModeToggle />
      ...
      <main className="mobile-bottom-space home-reveal mx-auto w-full max-w-6xl px-5 py-16 md:px-8 md:py-24">
        <section className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-zinc-500 dark:text-zinc-400">
            I’m <button type="button" onClick={onOpenContact} className="font-semibold text-zinc-700 underline decoration-dotted underline-offset-2 transition-colors hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-zinc-50" aria-label="Contact Stephen">Stephen</button>, and {copy.globalHome.description}
          </p>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredSubjects.map((subject) => (
                <button
                  key={subject.id}
                  type="button"
                  onClick={(event) => onSelectWorkspace(subject, event)}
                  className="home-card group flex min-h-60 flex-col rounded-2xl border border-zinc-200 bg-white p-6 text-left shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-zinc-400 hover:shadow-lg dark:border-white/[0.08] dark:bg-white/[0.03] dark:hover:border-white/25 dark:hover:bg-white/[0.05]"
                >
                  <div className="flex items-start justify-between gap-4">
                    <img src={logoSrc} alt="" className="size-10 rounded-xl" aria-hidden="true" />
                    <span className="rounded-full border border-zinc-200 px-2 py-1 font-mono text-[10px] text-zinc-500 dark:border-white/[0.1]">{subject.id}</span>
                  </div>
                  <div className="mt-auto pt-8">
                    <div className="mb-3 flex flex-wrap gap-1.5" aria-label="Project tags">
                      {subject.tags.map((tag) => (
                        <span key={tag} className="rounded-full bg-zinc-100 px-2 py-1 font-mono text-[9px] uppercase tracking-wide text-zinc-600 dark:bg-white/[0.07] dark:text-zinc-400">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <h3 className="text-lg font-semibold text-zinc-950 dark:text-zinc-50">{subject.label}</h3>
                    <p className="mt-2 text-sm leading-6 text-zinc-500 dark:text-zinc-400">{copy.globalHome.workspaces[subject.id as keyof typeof copy.globalHome.workspaces]}</p>
                    <span className="mt-5 inline-flex items-center gap-2 text-xs font-semibold text-zinc-900 dark:text-zinc-100">{copy.globalHome.openLabel}<ArrowRight className="size-3.5 transition-transform group-hover:translate-x-1" /></span>
                  </div>
                </button>
              ))}
            </div>
          ) : (
            <div className="rounded-2xl border border-dashed border-zinc-300 py-16 text-center text-sm text-zinc-500 dark:border-white/[0.12]">{copy.globalHome.noResults}</div>
          )}
        </section>
      </main>
    </div>
