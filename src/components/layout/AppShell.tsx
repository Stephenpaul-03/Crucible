...
export function AppShell() {
  ...
  // Welcome overlay & Support modal states
  const [isContactOpen, setIsContactOpen] = useState(false)
  const [isWelcomeOpen, setIsWelcomeOpen] = useState(() => {
    return localStorage.getItem("crucible-welcomed") !== "true"
  })

  // System menu preference state
  const [useSystemMenu, setUseSystemMenu] = useState(() => {
    return localStorage.getItem("crucible-use-system-menu") === "true"
  })

  const handleCloseWelcome = () => {
    localStorage.setItem("crucible-welcomed", "true")
    setIsWelcomeOpen(false)
  }

  const handleSwitchToSystemMenu = () => {
    localStorage.setItem("crucible-use-system-menu", "true")
    setUseSystemMenu(true)
  }

  const handleRestoreCustomMenu = () => {
    localStorage.setItem("crucible-use-system-menu", "false")
